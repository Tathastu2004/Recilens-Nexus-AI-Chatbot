...
Please analyze this document and answer the user's question based on the content above. Be specific and reference details from the document."""
        ...
        elif request.contextEnabled and model_manager:
            # ✅ RAG PIPELINE (only if services available)
            logger.info("📄 [RAG] RAG pipeline enabled and available")
            try:
                # Use ModelManager to get RAG response
                ...
                final_prompt = f"[RAG unavailable] {request.message}"
        
        elif request.contextEnabled and not model_manager:
            # ✅ RAG requested but not available
            logger.warning("⚠️ [RAG] RAG requested but services not available")
            final_prompt = f"Note: Document search is currently unavailable (RAG services not running). I'll provide a general response.\n\nUser question: {request.message}"
        ...
        if request.type == "document":
            system_msg = "You are a helpful AI assistant specialized in document analysis. When analyzing documents, be thorough and specific, referencing the actual content. Provide detailed insights about the document's content, structure, and key information."
        elif request.contextEnabled:
            system_msg = "You are a helpful AI assistant with access to company knowledge base. Use the provided context to give accurate and specific answers."
        else:
            system_msg = "You are a helpful AI assistant. Provide clear, accurate, and helpful responses."
        ...
        try:
            ...
            options = {
                "temperature": 0.3 if is_document else 0.7,
                "top_p": 0.9,
                "repeat_penalty": 1.1,
